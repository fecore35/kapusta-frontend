...
function Schedule({type,category}) {
    ...
    const sort = () => tick.sort((a, b) => {
        return b.sum - a.sum
    })
    const dataForSchedule = () => tick.sort((a, b) => {
        return b.sum - a.sum
    }).map(el => {
                            return {
                                quarter: el.name,
...
if (type !== "Расход") {
    ...
    useEffect(() => {
    console.log("Изменили категорию - получаем новые данные с БД по выбранной категории и перерисовываем график")
    },[category])
